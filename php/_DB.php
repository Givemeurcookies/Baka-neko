...
class DB{
	...
	public function login($user, $pass, $table = null) {
		...
		$sql = ("SELECT id, pass FROM UserLogin WHERE User='$user' LIMIT 1");
		$result = self::dosql($sql);
		if ($result['status'] == true) {
			$hashSQL = $result['result']['pass'];
			return (validate_password($pass, $hashSQL));
		}  else {
			// Might want to uncomment if something goes wrong
			//print_r ($result);
			return false;
		}
		return false;	
	...
	private function dosql($query){
		...
		try {
			...
			if ($result) { return array('status' => true, "result" => $result); } else { return array('status' => false); }
		} catch (PDOException $e) {
